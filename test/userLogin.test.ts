...
test.before(async () => {
  ...
  await t
    .expect(loginPage.loginFormErrorMessage.innerText)
...
test('Create user, navigate to login page and login', async t => {
  ...
  await t
    ...
    .expect(loginPage.notificationBar(await userUtils.getFullName(primaryUser)).exists)
    .ok();
  ...
  await t
    ...
    .eql(homePage.url)
    .navigateTo(loginPage.url)
    .expect(loginPage.notificationBar(await userUtils.getFullName(secondaryUser)).exists)
    .ok();
});
