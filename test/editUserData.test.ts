...
test('Change primaryUser data to secondaryUser', async t => {
  ...
  await t
    ...
    .expect(customerInfoPage.lastNameInput.value)
    .eql(secondaryUser.lastName)
    .expect(customerInfoPage.selectBDayDay.value)
    .eql(secondaryUser.dateOfBirth.day)
    .expect(customerInfoPage.selectBDayYear.value)
    .eql(secondaryUser.dateOfBirth.year)
    .expect(customerInfoPage.emailInput.value)
